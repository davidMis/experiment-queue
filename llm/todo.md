...
- [ ] **EQ-RELEASE-002 — David with Codex support:** commit and push the
  readiness candidate, including the currently local Linux Python 3.14 CI
  workflow. Depends on the completed local verification recorded in
  `llm/status.md` and `llm/log.md`. Complete when the remote branch names the
  reviewed commit and clean install, tests, packaging, and CLI smoke pass
  remotely without GPUs or operator state.
